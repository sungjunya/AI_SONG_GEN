...
router.post('/songs', async (req, res) => {
    ...
    try {
        ...
        await pool.execute(
            ...
            [decoded.userId, prompt, lyrics || null, audio_url || null]
        );
        res.status(201).json({ message: '노래 저장 성공' });
    } catch (e) {
        console.error('노래 저장 에러:', e);
        res.status(500).json({ error: '노래 저장 실패: ' + e.message });
    }
});
...
router.get('/songs', async (req, res) => {
    ...
    try {
        const decoded = jwt.verify(token, process.env.JWT_SECRET);
        const [rows] = await pool.execute(
            'SELECT id, prompt, lyrics, audio_url, created_at FROM songs WHERE user_id = ?',
            [decoded.userId]
        );
        res.json(rows);
    } catch (e) {
        console.error('노래 조회 에러:', e);
        res.status(500).json({ error: '노래 조회 실패: ' + e.message });
    }
});
